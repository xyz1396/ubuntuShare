# id 3300005512 failed because no file exists
# revised by getting uid by str_extract
# make it possible to use trycatch
...
getUID <- function(id, phantomPath = "phantomjs") {
  portalSearch <-
    "https://genome.jgi.doe.gov/portal/?core=genome&query="
  portalSearch <- paste0(portalSearch, id)
  portalSearch <- phantomJS(portalSearch, cookies, phantomPath)
  portalSearch <-
    str_extract(portalSearch, 'href="/.*\\.download\\.html"')
  portalSearch <- as.character(na.omit(portalSearch))
  uid <- str_split(portalSearch, "/", simplify = T)[3]
  # uid may be NA when get UId failed
  uid
}
...
#### get id.tar.gz link and md5 ####

getLinkAndMD5 <- function(id, XMLfile) {
