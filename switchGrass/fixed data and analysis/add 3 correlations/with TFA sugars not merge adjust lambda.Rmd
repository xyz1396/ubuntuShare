...
colnames(phy)[colnames(phy)=="total_sugar"]<-"Cellulose"
...
getPhy2Gene<-function(mat,fileName){
  ...
  # produce error; qvalue(p.gini$p,lambda = seq(0, max(p.gini$p)+1E-2, 1E-2)) 
  # the qvalue will be too small when just producing no error
  q<-qvalue(p.gini$p,lambda = seq(0, max(p.gini$p)-1E-2, 1E-2))
...
getGene2Gene<-function(mat,fileName){
  results<-getGiniMatrix(mat)
  r<-results$corMatrix[nGene:nMat,nGene:nMat]
  r <- cbind(r, physiology = rownames(r))
  r.gini <-
    pivot_longer(as.data.frame(r),
                 -physiology,
  ...
  p<-results$pvalueMatrix[nGene:nMat,nGene:nMat]
  p <- cbind(as.data.frame(p), physiology = rownames(p))
  p.gini <-
    pivot_longer(as.data.frame(p),
                 -physiology,
...
nGene<-ncol(gene)
nMat<-ncol(mat)
getPhy2Gene(mat,"Physiology2Gene.gini.csv")
```
...
getPhy2Gene(mat,"Genotypes only; A4; N(GE) vs N(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; A4; N(GE) vs N + 1(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; A4; N(GE) vs N + 2(CW).gini.csv")
```
...
getPhy2Gene(mat,"Genotypes only; AP13; N(GE) vs N(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; AP13; N(GE) vs N + 1(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; AP13; N(GE) vs N + 2(CW).gini.csv")
```
...
getPhy2Gene(mat,"Genotypes only; VS16; N(GE) vs N(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; VS16; N(GE) vs N + 1(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"Genotypes only; VS16; N(GE) vs N + 2(CW).gini.csv")
```
...
mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"S1 vs R3.csv")
```

### S2 vs R3

```{r}
gene3<-getSub(gene2,c("S2"))
phy3 <- getSub(phy2,c("R3"))
sum(gene3$GeneType==phy3$GeneType)

mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"S2 vs R3.csv")
```

### S3 vs R3

```{r}
gene3<-getSub(gene2,c("S3"))
phy3 <- getSub(phy2,c("R3"))
sum(gene3$GeneType==phy3$GeneType)

mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"S3 vs R3.csv")
```

### R3 vs R3

```{r}
gene3<-getSub(gene2,c("R3"))
phy3 <- getSub(phy2,c("R3"))
sum(gene3$GeneType==phy3$GeneType)

mat<-cbind(gene3[,-1:-3],phy3[,-1:-3])
getPhy2Gene(mat,"R3 vs R3.csv")
```