...
for (i in fileName){
  ...
  GCC<-GCC[,-1]
  write.xlsx(GCC,"QandFDRtoMatrix.xlsx",paste0(sheetName,"_GCC"),append = T)
  
  p_value<-pivot_wider(df[,c(1,2,4)],names_from = gene,values_from = p)
  p_value<-as.data.frame(p_value)
  rownames(p_value)<-p_value$physiology
  p_value<-p_value[,-1]
  write.xlsx(p_value,"QandFDRtoMatrix.xlsx",paste0(sheetName,"_p_value"),append = T)
  ...
  q_value<-q_value[,-1]
  write.xlsx(q_value,"QandFDRtoMatrix.xlsx",paste0(sheetName,"_q_value"),append = T)
  
  BH_FDR<-pivot_wider(df[,c(1,2,6)],names_from = gene,values_from = BH)
  BH_FDR<-as.data.frame(BH_FDR)
  rownames(BH_FDR)<-BH_FDR$physiology
  BH_FDR<-BH_FDR[,-1]
  write.xlsx(BH_FDR,"QandFDRtoMatrix.xlsx",paste0(sheetName,"_BH_FDR"),append = T)
}
```
