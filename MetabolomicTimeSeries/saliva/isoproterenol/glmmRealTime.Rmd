...
df <-
  read.csv(
    "../Biomarkers Full Data Set_quant_BlkRemoved_missedinjRemoved_misslabeledRemoved_T_wMeta_TIC_saliva_20210520.csv"
    ,
    stringsAsFactors = F
  )
df <- df[df$ATTRIBUTE_condition != "infected",]
...
# remove data points with 60% missing value under isoproterenol treatment
dataISO <- data[meta$ATTRIBUTE_condition == "isoproterenol", ]
dataISO[dataISO == 0] <- NA
colNA <- colSums(is.na(dataISO)) / nrow(dataISO)
